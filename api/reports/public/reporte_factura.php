...
if ($pedido->setId($_GET['idPedido'])) {
    ...
    if ($rowOne = $pedido->readOne()) {
        ...
        $pdf->ln(12);
        $pdf->setFont('Arial', 'B', 10);
        $pdf->cell(0, 0, 'Fecha en que se hizo: ' . $rowOne['fecha']);
        // Se agrega un salto de línea
        $pdf->ln(10);
        ...
        if ($dataP = $pedido->readFactura()) {
            ...
            $pdf->cell(78.24, 10, 'Producto', 'B', 0, 'C', 1);
            $pdf->cell(39.12, 10, 'Marca', 'B', 0, 'C', 1);
            $pdf->cell(29.34, 10, 'Categoria', 'B', 0, 'C', 1);
            $pdf->cell(39.12, 10, 'Cantidad', 'B', 1, 'C', 1);
            ...
            foreach ($dataP as $rowP) {
                ...
                $pdf->cell(78.24, 10, $pdf->encodeString($rowP['nombre_producto']), 'TB', 0, 'C');
                $pdf->cell(39.12, 10, $pdf->encodeString($rowP['nombre_marca']), 'TB', 0, 'C');
                $pdf->cell(29.34, 10, $pdf->encodeString($rowP['nombre_categoria']), 'TB', 0, 'C');
                $pdf->cell(39.12, 10, $pdf->encodeString($rowP['cantidad_producto']), 'TB', 1, 'C');
            }
